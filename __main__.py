...
from app.psis import get_pesticide
...
def render_shop_query(crop_name: str, disease: str) -> str:
    """네이버 쇼핑 검색 키워드를 생성합니다

    - 병해충명 중 작물명이 포함된 경우 작물명을 삭제합니다.
    - 병해충명 앞에 작물명을 추가합니다
    """
    disease = disease.replace(crop_name, "")
    return f"{crop_name} {disease}"
...
def render_dataframes_to_text(crop_name: str, df: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        ...
        # 농약등록정보로부터 받아온 농약 정보
        "pesticides": get_pesticide(crop_name=crop_name, disease=disease),
        # 네이버 쇼핑 검색어
        "shop_query": render_shop_query(crop_name=crop_name, disease=disease),
    }
...
def predict() -> str | BaseResponse:
    ...
    # 사용자가 입력한 작물명에 알맞는 가중치 모델을 가져옵니다
    model = models.get(crop_name)
    if not model:
        return redirect(url_for("index"))
